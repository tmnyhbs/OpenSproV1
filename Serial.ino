...
void setupSerialInterface()  {
  ...
  void printHelp() {
    ...
    Serial.println("E : Activate Group (w/Preinfusion");
    Serial.println("e : Deactivate Group");
    Serial.println("S : Activate Steam");
    Serial.println("s : Deactivate Steam");
    Serial.println("G : Steam Wand Purge");
  ...
  void updateSerialInterface() {
    while (Serial.available()) {
      ...
      if (incomingByte == 'E') {
        ...
        Serial.println("Brew Group Pre Infusing");
        delay(PREINF_DELAY);
        digitalWrite(PUMP, HIGH);
        Serial.println("Brew Group Pressurized");
      }
      if (incomingByte == 'e') {
        digitalWrite(PUMP, LOW);
        digitalWrite(BREW, LOW);
        Serial.println("Brew Group Deactivated");
      }
      if (incomingByte == 'S') {
        digitalWrite(STEAM, HIGH);
        Serial.println("Steam Valve Activated");
      }
      if (incomingByte == 'G') {
        digitalWrite(STEAM, HIGH);
        Serial.println("Steam Valve Purge");
        delay(500);
  ...
  void printStatus() {
    ...
    printFloat(getTemp(), PRINT_PLACES_AFTER_DECIMAL);
